...
from utilInterpret import translate_kw
...
if baseline in ["jll", "entr", "vcluster"]:
    ...
    if perceiver == 0:
        from model import ViTEviContra as get_compiled_model
    else:
        from model import ViTEviPerceiverkw as get_compiled_model
    sparselabel = True
    model = get_compiled_model(
        sequence_length=Tvid,
        cap_length=Tcap,
    ...
    model.compile(
        ...
        metrics={"nll1": "mae", "kl1": "mae", "nll2": "mae", "kl2": "mae"},
    )
...
from utilInterpret import logging, logging2
from utilInterpret import (
    decode_greedy,
    decodeEvi_multisample,
    decodeEvi_topp,
    decode_unc,
    decodeCE_topp,
    decodeCE_beam,
